...
systemModels<- list(Model(population ~ Poisson(mean ~ age + time + age*time, useExpose=FALSE),
                          ...
                          age ~ Exch(),
                          jump = 0.002),
                    Model(births ~ Poisson(mean ~ age + time),
                          time ~ DLM(),
                          age ~ Exch(),
                          jump = 0.005),
                    Model(deaths ~ Poisson(mean ~ age + time),
                          time ~ DLM(),
                          age ~ Exch(),
                          jump = 0.01),
                    Model(external_in ~ Poisson(mean ~ age + time),
                          time ~ DLM(),
                          age ~ Exch(),
                          jump = 0.01),
                    Model(external_out ~ Poisson(mean ~ age + time),
                          time ~ DLM(),
                          age ~ Exch(),
                          jump = 0.03))
...
dataModels <- list(Model(census_year_erp ~ NormalFixed(mean = mean, sd = sd),
                         series = "population"),
                   Model(reg_births ~ Poisson(mean ~  age + time),
                         time ~ DLM(),
                         age ~ Exch(),
                         series = "births",
                         jump = 0.005),
                   Model(reg_deaths ~ Poisson(mean ~ age + time),
                         time ~ DLM(),
                         age ~ DLM(),
                         series = "deaths",
                         jump = 0.005),
                   Model(arrivals ~ Poisson(mean ~ age),
                         ...
                         series = "external_in",
                         jump = 0.01),
                   Model(departures ~ Poisson(mean ~ time + age),
                         age ~ Exch(),
...
onlyage <- "C:\\0_PhD\\Thesis\\Thesis_R\\AT6.est"
...
beepe(sound=2, estimateAccount(account = account,
