...
cy1 <- Counts(census_year_erp %>%
                subarray(time < "2013"),  dimscales = c(time="Points"))

cy2 <-  Counts(census_year_erp %>%
                 subarray(time >= "2013"), dimscales = c(time="Points"))
...
dataModels <- list(Model(census_year_erp1 ~ NormalFixed(mean = mean, sd = sd),
                         series = "population"),
                   Model(census_year_erp2 ~ PoissonBinomial(prob = 0.90),
                         series = "population"),
...
filename <- "C:/0_PhD/Thesis/Thesis_R/onlytime3.est"
...
plot(MCMC$dataModels.arrivals.prior.sd, sub=names(MCMC)[31]) #...convergence?
plot(MCMC$dataModels.departures.likelihood.rate, sub=names(MCMC)[32]) # most problematic chain
plot(MCMC$dataModels.departures.prior.mean, sub=names(MCMC)[33]) # prior ok
