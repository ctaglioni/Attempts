...
dataModels <- list(Model(census_year_erp ~ NormalFixed(mean = mean, sd = sd),
                         series = "population"),
                   Model(reg_births ~ Poisson(mean ~ age + time + age:time),
                         time ~ DLM(),
                         age ~ DLM(),
                         series = "births",
                         jump = 0.005),
                   Model(reg_deaths ~ Poisson(mean ~ age + time + age:time),
                         time ~ DLM(),
                         age ~ DLM(),
                         series = "deaths",
                         jump = 0.005),
                   Model(arrivals ~ Poisson(mean ~ age),
                         age ~ DLM(),
                         series = "external_in",
                         jump = 0.01),
                   Model(departures ~ Poisson(mean ~ age),
                         age ~ DLM(trend = Trend(scale = HalfT(scale = 0.2))),
                         series = "external_out",
                         jump = 0.01))


onlyage <- "C:\\0_PhD\\Thesis\\Thesis_R\\AT2.est"
