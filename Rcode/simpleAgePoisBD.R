...
summary(account)
systemModels<- list(Model(population ~ Poisson(mean ~ time + age, useExpose=FALSE),
                          time ~ DLM(damp = NULL),
                          age ~ DLM(trend = NULL),
                          jump = 0.002),
                    Model(births ~ Poisson(mean ~ time + age),
                          time ~ DLM(trend = NULL, damp = NULL),
                          age ~ DLM(trend = NULL),
                          jump = 0.006),
                    Model(deaths ~ Poisson(mean ~ time + age),
                          time ~ DLM(trend = NULL, damp = NULL),
                          age ~ DLM(trend = NULL),
                          jump = 0.015),
                    Model(external_in ~ Poisson(mean ~ time + age),
                          time ~ DLM(trend = NULL),
                          age ~ DLM(trend = NULL),
                          jump = 0.02),
                    Model(external_out ~ Poisson(mean ~ time + age),
                          time ~ DLM(trend = NULL),
                          age ~ DLM(trend = NULL),
                          jump = 0.03))


# Model with only 5 years, i.e. one period
# 
# systemModels<- list(Model(population ~ Poisson(mean ~ age, useExpose=FALSE),
#                           age ~ DLM(trend = NULL)),
#                     Model(births ~ Poisson(mean ~ age),
#                           age ~ DLM(trend = NULL)),
#                     Model(deaths ~ Poisson(mean ~ age),
#                           age ~ DLM(trend = NULL)),
#                     Model(external_in ~ Poisson(mean ~ age),
#                           age ~ DLM(trend = NULL)),
#                     Model(external_out ~ Poisson(mean ~ age),
#                           age ~ DLM(trend = NULL)))
...
dataModels <- list(Model(census_year_erp ~ PoissonBinomial(prob = 0.95),
                         series = "population"),
                   Model(reg_births ~ Poisson(mean ~ 1),
                         series = "births"),
                   Model(reg_deaths ~ Poisson(mean ~ 1),
                         series = "deaths"),
                   Model(arrivals ~ Poisson(mean ~ 1),
                         series = "external_in",
                         jump = 0.015),
...
onlyage <- "C:\\0_PhD\\Thesis\\Thesis_R\\simpleagePoisBD.est"
