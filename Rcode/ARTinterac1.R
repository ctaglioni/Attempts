...
setwd("C:\\0_PhD\\Thesis\\GIT\\Attempts\\Rcode")
...
population<- census_year_erp[ , , c(1,6,11)]
dim(population)
...
systemModels<- list(Model(population ~ Poisson(mean ~ age + time + region, useExpose=FALSE),
                    ...
                    Model(deaths ~ Poisson(mean ~ age + time + region),
                          ...
                          age ~ DLM(),
                          jump = 0.015),
                    Model(external_in ~ Poisson(mean ~ age + time + region + age*region),
                          time ~ DLM(trend = NULL), 
                          region ~ Exch(),
                          age ~ DLM(),
                          jump = 0.03),
                    Model(external_out ~ Poisson(mean ~ age + time + region + age*region),
                          time ~ DLM(trend = NULL),
                          region ~ Exch(),
                          age ~ DLM(),
                          jump = 0.03),
                    Model(internal_in ~ Poisson(mean ~ age + time + region + age*region),
                          time ~ DLM(trend = NULL), 
                          region ~ Exch(),
                          age ~ DLM(),
                          jump = 0.03),
                    Model(internal_out ~ Poisson(mean ~ age + time + region + age*region),
                          time ~ DLM(trend = NULL),
                          region ~ Exch(),
                          age ~ DLM(),
                          jump = 0.03))
...
dataModels <- list(Model(census_year_erp ~ PoissonBinomial(prob = 0.95),
                   ...
                   Model(reg_deaths ~ PoissonBinomial(prob = 0.98),
                         series = "deaths"),
                   Model(arrivals ~ Poisson(mean ~ region * age),
                         series = "external_in",region ~ Exch(),
                         age ~ DLM(),
                         jump = 0.05),
                   Model(departures ~ Poisson(mean ~ region * age),
                         series = "external_out",
                         region ~ Exch(),
                         age ~ DLM(),
                         jump = 0.05),
                   Model(intarr ~ Poisson(mean ~ region * age),
                         series = "internal_in",
                         region ~ Exch(),
                         age ~ DLM(),
                         jump = 0.05),
                   Model(intdep ~ Poisson(mean ~ region * age),
                         series = "internal_out",
                         region ~ Exch(),
                         age ~ DLM(),
                         jump = 0.05))

onlyreg <- "C:/0_PhD/Thesis/Thesis_R/ARTinterac1.est"
