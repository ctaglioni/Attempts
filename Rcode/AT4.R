...
systemModels<- list(Model(population ~ Poisson(mean ~ age + time + age*time, useExpose=FALSE),
                          ...
                          age ~ DLM(),
                          jump = 0.002),
                    Model(births ~ Poisson(mean ~ age + time + age*time),
                          time ~ DLM(),
                          age ~ DLM(),
                          jump = 0.005),
                    Model(deaths ~ Poisson(mean ~ age + time + age*time),
                          time ~ DLM(),
                          age ~ DLM(),
                          jump = 0.01),
                    Model(external_in ~ Poisson(mean ~ age + time + age*time),
                          time ~ DLM(),
                          age ~ DLM(),
                          jump = 0.01),
                    Model(external_out ~ Poisson(mean ~ age + time + age*time),
                          time ~ DLM(),
                          age ~ DLM(),
                          jump = 0.03))
...
onlyage <- "C:\\0_PhD\\Thesis\\Thesis_R\\AT4.est"
