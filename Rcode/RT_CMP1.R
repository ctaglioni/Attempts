...
systemModels<- list(Model(population ~ Poisson(mean ~ time + region + time*region, useExpose=FALSE),
                    ...
                    Model(births ~ Poisson(mean ~ time + region + time*region),
                          time ~ DLM(),
                          region ~ Exch(error = Error(scale = HalfT(scale = 0.05))),
                          time:region ~ Exch(),
                          jump = 0.02),
...
dataModels <- list(Model(census_year_erp1 ~ NormalFixed(mean = mean, sd = sd),
                   ...
                   Model(reg_deaths ~ PoissonBinomial(prob = 0.90),
                         series = "deaths"),
                   Model(arrivals ~ CMP(mean ~ region,
                                        dispersion = Dispersion(mean = Norm(mean = 0,
                                                                            sd = 0.2),
                                                                scale = HalfT(scale = 0.05))),
                         series = "external_in"),
                   Model(departures ~ NormalFixed(mean = mean4, sd = sd4),
                         region ~ Exch(),
                         series = "external_out"),
...
onlyreg <- "C:/0_PhD/Thesis/Thesis_R/RT_CMP2.est"
...
plot(MCMC$dataModels.arrivals.prior.dispersion.mean)
plot(MCMC$dataModels.arrivals.prior.dispersion.sd)
