...
dataModels <- list(Model(census_year_erp ~ NormalFixed(mean = mean, sd = sd),
                         series = "population"),
                   Model(reg_births ~ Poisson(mean ~ age),
                         age ~ Exch(),
                         series = "births",
                         jump = 0.005),
                   Model(reg_deaths ~ Poisson(mean ~ age ),
                         age ~ DLM(level = Level(scale = HalfT(scale = 0.1))),
                         series = "deaths",
                         jump = 0.005),
                   Model(arrivals ~ Poisson(mean ~ age + time),
                         time~ DLM(),
                         age ~ DLM(),
                         series = "external_in",
                         jump = 0.01),
                   Model(departures ~ Poisson(mean ~ age + time),
...
onlyage <- "C:\\0_PhD\\Thesis\\Thesis_R\\AT3.est"
